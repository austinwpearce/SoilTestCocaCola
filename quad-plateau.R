# Fit a quadratic plateau segmented regression to XY data
...
library(minpack.lm) # for nlsLM, a backup to nls 
...
quad_plateau <- function(data, resid = FALSE, plot = FALSE, band = FALSE, percent_of_max = 95) {
    ...
    corr_model <- try(nls(y ~ SSquadp3xs(x, b0, b1, jp), data = data))
    
    if (inherits(corr_model, "try-error")) {
        corr_model <- try(minpack.lm::nlsLM(y ~ SSquadp3xs(x, b0, b1, jp), data = data))
    } else {
        ...
        qp_plot <- predicted %>%
            ...
            {if (band == TRUE) 
                geom_ribbon(aes(y = Estimate,
                                ...
                                ymax = Q97.5),
                            alpha = 0.05)} +
            geom_vline(xintercept = jp, alpha = 0.5, linetype = 3) +
            geom_hline(yintercept = plateau, alpha = 0.5, linetype = 3) +
            geom_line(
                stat="smooth",
                method = "nls",
            ...
            annotate(
                ...
                x = join_point, y = 0, angle = 90,
                hjust = 0, vjust = 1.5, alpha = 0.5
            ) +
            annotate(
                "text",
                label = paste0("Plateau = ", round(plateau,1), "%"),
                x = maxx, y = plateau,
                alpha = 0.5, hjust = 1, vjust = 1.5
            ) +
            annotate("text",
